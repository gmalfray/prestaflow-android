...
class FakeDashboardRepository : DashboardRepository {
    ...
    private fun flowForPeriod(period: DashboardPeriod) = snapshotByPeriod.getOrPut(period.name) { MutableStateFlow(null) }

    private fun flowForCustom(
        from: String,
        to: String,
    ) = snapshotByCustom.getOrPut("$from:$to") { MutableStateFlow(null) }

    override fun observeDashboard(period: DashboardPeriod): Flow<DashboardSnapshot?> = flowForPeriod(period)

    override fun observeCustomDashboard(
        from: String,
        to: String,
    ): Flow<DashboardSnapshot?> = flowForCustom(from, to)
