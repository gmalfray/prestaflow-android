...
- Objectif : une application Android (minSdk 26, targetSdk 35) qui gère entièrement une boutique PrestaShop ≥ 1.7 via le module Rebuild Connector (REST JSON) et, à terme, la nouvelle API PrestaShop 9.
...
- Module PrestaShot : page de configuration (BO > Modules > PrestaShot) où l’on saisit la clé API et génère un QR code encodant `{"version":1,"shopUrl":...,"apiKey":...}`. Le QR code est consommé par l’application mobile via le schéma `prestaflow://setup?data=<base64(JSON)>`; prévoir rotation manuelle de la clé en cas de révocation.
...
Fallback legacy (`mobassistantconnector`) conservé comme plan B, mais désactivé par défaut. L’app détecte la compatibilité en appelant `/connector/login`; si 404 ou 410, afficher un message qui invite à mettre le module à jour.
...
- Qualité : activer Compose metrics (`enableComposeCompilerReports=true`), `Detekt`, `ktlint`, `Gradle Doctor`. Prévoir tests unitaires (usecases, repos) + tests instrumentés (flows critiques). Ajouter des tests snapshot Compose si utile.
...
- Stockage local : `EncryptedSharedPrefs` via `TokenManager` + `TokenStorage` (InMemory + disque), plus `BiometricPrompt` optionnel pour un déverrouillage rapide. Purger dès le logout ou l’expiration. Ne jamais stocker de clés en clair, ni de logs contenant des PII.
...
- Audit : aligner avec `rebuild-connector` (logs HMAC, allowlist IP). L’app doit honorer `scopes` (masquer les modules non autorisés).

## 10. Checklists
- Avant une release :
  - Vérifier versionName/versionCode, changelog FR/EN.
  - Lancer `lint`, `detekt`, `ktlint`, tests unitaires, tests instrumentés principaux.
